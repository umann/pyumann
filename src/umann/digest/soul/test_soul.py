...
from umann.digest import Soul, extract_soul
from umann.digest.soul import NoHandlerError
...
def test_jpg_plugin():
    ...
    jpg_data = (
        ...
        b"IMAGEDATA"  # Image data (9 bytes)
    ...
    assert s.offset == 20
    # Soul should include from SOS to end: 10 + 9 + 2 = 21
    assert s.length == 21
...
def test_mp3_plugin():
    ...
    # Syncsafe encoding for size 10: 0x00 0x00 0x00 0x0a -> already syncsafe
    mp3_data = (
        b"ID3"  # ID3v2 identifier
        + b"\x04\x00"  # Version 2.4
        + b"\x00"  # Flags
        + b"\x00\x00\x00\x0a"  # Syncsafe size (10 bytes) - correct
        + b"Y" * 10  # ID3 tag data (10 bytes)
        + b"\xff\xfb"  # MP3 frame sync
        + b"AUDIODATA"  # Audio data
    )
...
def test_avi_plugin():
    ...
    # Soul should start at LIST/movi (after RIFF header: 4 + 4 + 4 = 12)
    assert s.offset == 12
    assert b"LIST" in s.result("soul")
    assert b"movi" in s.result("soul")
...
def test_no_handler_error():
    """Test error when no plugin can handle file."""

    unknown_data = b"UNKNOWN_FORMAT_12345"

    with pytest.raises(NoHandlerError):
        Soul(content=unknown_data).compute()
