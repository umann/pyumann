...
from umann.utils.digest.soul import SoulError, SoulPlugin
    ...
    def handle(self) -> None:
        ...
        try:
            ...
            while s.pos < s.size - 8:
                ...
                if atom_size == 1:
                    if s.pos + 8 > s.size:
                        raise SoulError(f"{fname}: Truncated extended size atom")
                ...
                if atom_type == b"mdat":
                    ...
                    if s.offset + s.length > s.size:
                        raise SoulError(f"{fname}: mdat atom extends beyond file: {s.offset + s.length} > {s.size}")
            ...
            raise SoulError(f"{fname}: No mdat atom found in ISO BMFF file")

        except SoulError:
            # File is corrupted or incomplete - mark as soulless
            s.soulless = True
